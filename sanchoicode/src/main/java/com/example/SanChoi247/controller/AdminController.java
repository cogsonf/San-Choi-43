...
public void approveRefund(int booking_id) throws Exception {
    ...
    public ResponseEntity<List<User>> getPendingRequests() {
        ...
        @GetMapping("/admin/pending-coach-requests")
        public List<User> getPendingCoachRequests() throws Exception {
        return userRepo.findUsersByStatus(7);
        }

@PostMapping("/approve-coach")
public String approveCoach(@RequestParam("uid") int uid) {
    try {
        User user = userRepo.getUserById(uid);
        user.setStatus(0); // 1 = approved
...
@PostMapping("/reject-coach")
public String rejectCoach(@RequestParam("uid") int uid) {
    try {
        User user = userRepo.getUserById(uid);
        user.setStatus(0); // 0 = rejected or normal user
        userRepo.updateUserById(user);
        return "redirect:/coach-requests";
    } catch (Exception e) {
        e.printStackTrace();
